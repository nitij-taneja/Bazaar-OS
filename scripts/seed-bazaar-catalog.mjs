...
try {
  ...
  for (const [position, item] of catalog.products.entries()) {
    ...
    await tx.execute({
      ...
      args: [productId, testPriceInrPaise, testInventory, JSON.stringify(cities), "Next-day in demo service areas", JSON.stringify(tags), JSON.stringify(occasions), testOverlayLabel, "Test price uses a documented USD-to-INR staging conversion of ₹83.00 per public-source USD. Inventory and delivery are deterministic staging values used only to demonstrate bounded Razorpay test-mode commerce."],
    });
    ...
    const facts = [
      ...
      ["delivery", `Next-day in ${cities.join(", ")}`, "operational_overlay", "BazaarOS staging delivery policy", "deterministic_overlay"],
  ...
  await tx.commit();
  console.log(`Seeded ${catalog.products.length} public-source catalog items for merchant ${merchantId}.`);
} catch (error) {
  await tx.rollback();
  throw error;
} finally {
