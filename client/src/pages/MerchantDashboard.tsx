...
function MerchantWorkspace() {
  ...
  const toggleSponsorship = trpc.commerce.toggleSponsorship.useMutation({
    onSuccess: data => { setMessage(data.isSponsored ? "Sponsorship enabled — this listing gets a disclosed ranking boost." : "Sponsorship disabled."); void utils.commerce.merchantCatalog.invalidate(); },
    onError: error => setMessage(error.message),
  });
