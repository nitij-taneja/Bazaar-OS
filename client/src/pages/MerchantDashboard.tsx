...
import { CheckCircle2, CircleAlert, Database, FileUp, Network, PackagePlus, RefreshCw, Sparkles, TrendingDown, Upload, WalletCards } from "lucide-react";
...
function MerchantWorkspace() {
  ...
  const toggleSponsorship = trpc.commerce.toggleSponsorship.useMutation({
    onSuccess: data => { setMessage(data.isSponsored ? `Sponsorship enabled with a ₹${((data.sponsorBudgetInrPaise ?? 0) / 100).toFixed(0)} campaign budget — this listing gets a disclosed ranking boost until that budget runs out.` : "Sponsorship disabled. Its last campaign's spend stays visible."); void utils.commerce.merchantCatalog.invalidate(); },
    onError: error => setMessage(error.message),
  });
  const growthInsights = trpc.commerce.growthInsights.useQuery();
  const agentInsights = trpc.commerce.merchantAgentInsights.useQuery(undefined, { refetchInterval: 8_000 });
  ...
  return <div className="min-h-screen bg-[#060914] text-slate-100"><div className="mx-auto max-w-6xl space-y-7 p-4 sm:p-8"><header className="flex flex-col gap-4 rounded-3xl border border-emerald-400/20 bg-gradient-to-r from-emerald-950/70 via-[#0b1b13]/80 to-[#070e1c]/90 p-6 text-white shadow-[0_22px_70px_rgba(5,20,12,.4)] backdrop-blur-xl sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-bold tracking-[.16em] text-emerald-300"><Sparkles size={14} /> NOVACART · MERCHANT COMMAND CONSOLE</div><h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">Manage NovaCart catalog. Let AI sell it securely.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-emerald-100/75">Products added here remain visibly marked as merchant-supplied facts, receive a real semantic vector for product discovery, and stay behind BazaarOS payment safeguards.</p></div><Link href="/"><Button variant="outline" className="border-white/20 bg-white/10 text-white hover:bg-white/20 hover:text-white">Open customer experience</Button></Link></header>
    ...
    <section className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]"><div className="rounded-3xl border border-white/10 bg-[#0c1222]/90 p-5 shadow-xl backdrop-blur sm:p-6"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold text-white">Your catalog</h2><p className="mt-1 text-sm text-slate-400">Inventory is a merchant-managed Test Mode overlay. Source provenance remains immutable.</p></div><Button onClick={() => reindex.mutate()} disabled={reindex.isPending} variant="outline" className="border-white/15 bg-white/[0.03] text-slate-200 hover:bg-white/10 hover:text-white"><RefreshCw size={15} className={`mr-2 text-cyan-300 ${reindex.isPending ? "animate-spin" : ""}`} /> Reindex semantic search</Button></div><div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-black/20"><div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 bg-white/[0.04] px-4 py-3 text-xs font-bold uppercase tracking-wider text-slate-400"><span>Product</span><span>Inventory</span><span>Sponsor</span><span>Action</span></div>{catalog.data?.productPreview?.map((product: any) => <div key={product.id} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-3 border-t border-white/5 px-4 py-3"><div className="min-w-0"><p className="truncate text-sm font-medium text-white">{product.title}</p><p className="mt-0.5 text-xs text-slate-400">{product.overlayLabel}</p></div><span className="rounded-lg border border-white/10 bg-white/[0.06] px-2.5 py-1 text-xs font-semibold text-emerald-200">{product.testInventory}</span><div className="flex flex-col gap-1"><Button size="sm" onClick={() => toggleSponsorship.mutate({ productId: product.id, isSponsored: !product.isSponsored, sponsorBudgetInrPaise: product.isSponsored ? undefined : Math.max(Number(budgetDrafts[product.id] || 500), 1) * 100 })} disabled={toggleSponsorship.isPending} variant="outline" className={`h-8 text-xs ${product.isSponsored ? "border-amber-400/40 bg-amber-400/10 text-amber-200 hover:bg-amber-400/20" : "border-white/15 bg-white/[0.03] text-slate-200 hover:bg-white/10 hover:text-white"}`}>{product.isSponsored ? "Sponsored ✓" : "Sponsor"}</Button>{product.isSponsored ? <p className="text-[10px] text-slate-400">₹{((product.sponsorSpentInrPaise ?? 0) / 100).toFixed(0)} of ₹{((product.sponsorBudgetInrPaise ?? 0) / 100).toFixed(0)} spent</p> : <Input value={budgetDrafts[product.id] ?? "500"} onChange={event => setBudgetDrafts({ ...budgetDrafts, [product.id]: event.target.value })} inputMode="numeric" className="h-6 w-20 border-white/10 bg-white/[0.04] px-1.5 text-[10px] text-white" placeholder="₹ budget" />}</div><Button size="sm" onClick={() => updateInventory.mutate({ productId: product.id, inventory: product.testInventory + 1 })} disabled={updateInventory.isPending} variant="outline" className="h-8 border-white/15 bg-white/[0.03] text-xs text-slate-200 hover:bg-white/10 hover:text-white">+1</Button></div>)}</div></div>
    ...
    <section className="rounded-3xl border border-white/10 bg-[#0c1222]/90 p-5 shadow-xl backdrop-blur sm:p-6"><div className="flex items-center justify-between"><div><h2 className="text-lg font-semibold text-white">My Agent</h2><p className="mt-1 text-sm text-slate-400">How your listing actually performed in the live cross-merchant marketplace — every bid your agent made, win or lose.</p></div><Network className="text-cyan-400" /></div>
      <div className="mt-5 grid gap-4 sm:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Marketplace bids</p><p className="mt-2 text-2xl font-semibold text-white">{agentInsights.data?.totalBids ?? 0}</p></div>
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Win rate</p><p className="mt-2 text-2xl font-semibold text-emerald-300">{agentInsights.data?.winRatePercent ?? 0}%</p></div>
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Avg. counter-offer used</p><p className="mt-2 text-2xl font-semibold text-amber-300">{((agentInsights.data?.avgDiscountAppliedPct ?? 0) * 100).toFixed(1)}%</p></div>
        <div className="rounded-2xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Sponsored bids</p><p className="mt-2 text-2xl font-semibold text-violet-300">{agentInsights.data?.sponsoredBidCount ?? 0}</p></div>
      </div>
      <div className="mt-5 space-y-2">
        {agentInsights.isLoading ? <div className="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-slate-400">Loading marketplace history…</div> : agentInsights.data?.recentBids.length ? agentInsights.data.recentBids.map((bid: any, index: number) => (
          <div key={index} className={`rounded-xl border p-3 ${bid.won ? "border-emerald-400/30 bg-emerald-950/20" : "border-white/10 bg-black/20"}`}>
            <div className="flex items-center justify-between gap-2">
              <p className="truncate text-sm font-medium text-white">{bid.productTitle}</p>
              <div className="flex shrink-0 items-center gap-1.5">
                {bid.isSponsored ? <Badge className="border-violet-300/30 bg-violet-300/10 text-violet-200">Sponsored +{Number(bid.sponsorBoostApplied ?? 0).toFixed(2)}</Badge> : null}
                <Badge className={bid.won ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-200" : "border-white/15 bg-white/5 text-slate-300"}>{bid.won ? "WON" : "LOST"}</Badge>
              </div>
            </div>
            <p className="mt-1 text-xs text-slate-400">
              {bid.merchantsConsidered} merchant{bid.merchantsConsidered === 1 ? "" : "s"} competed · ₹{((bid.finalPriceInrPaise ?? 0) / 100).toFixed(0)}
              {bid.discountAppliedPct > 0 ? ` (${(bid.discountAppliedPct * 100).toFixed(1)}% counter-offer applied)` : ""}
            </p>
          </div>
        )) : <div className="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-slate-400">No marketplace bids yet. Run a query in Marketplace Mode from the Studio to see this merchant's agent compete.</div>}
      </div>
      {agentInsights.data?.recentAcknowledgments.length ? (
        <div className="mt-5 border-t border-white/10 pt-4">
          <p className="text-xs font-bold uppercase tracking-wider text-slate-400">Recently acknowledged orders</p>
          <div className="mt-2 space-y-1.5">
            {agentInsights.data.recentAcknowledgments.slice(0, 5).map((ack: any, index: number) => (
              <p key={index} className="text-xs text-slate-300">✓ Order for product #{ack.productId} acknowledged and handed off for fulfillment.</p>
            ))}
          </div>
        </div>
      ) : null}
